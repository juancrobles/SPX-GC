...
router.post('/readExcelData', async (req, res) => {
  ...
  var excelFile = path.join(config.general.assetsfolder, req.body.filename);
  var workSheetsData;
  try {
...
router.post('/exportCSVfile', async (req, res) => {
  ...
  try {
    ...
    let CSVfileRef = path.join(config.general.assetsfolder, 'csv', filenameref + '_' + timestamp + '.csv');
